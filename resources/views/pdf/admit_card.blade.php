...
<head>
    ...
    <style type="text/css">
        ...
        .admit-card-body .common-subjects {
            width: 50%;
        }

        .admit-card-body .common-subjects h6 {
            margin-left: 28px;
            margin-bottom: 0px;
        }

        .admit-card-body .main-subjects {
            width: 50%;
        }


        .admit-card-body .main-subjects h6 {
            margin-left: -10px;
            margin-bottom: 0px;
        }
    ...
    <div class="admit-card-body">
        ...
        <table class="subjects">
            <tr>
                ...
                <td class="main-subjects">
                    <ul>
                        <h6>4th Subject:</h6>
                        @foreach ($fourth_subjects ?? [] as $key => $sub)
                            <li>
                                {{ $sub->subject->name_en ?? '' }}
                            </li>
                        @endforeach
                    ...
                    <ul>
                        <h6>Main Subject:</h6>
                        @foreach ($main_subjects ?? [] as $key => $sub)
                            <li>
                                {{ $sub->subject->name_en ?? '' }}
                            </li>
                        @endforeach
        ...
        @if (!empty($admit->issue_date ?? ($admitCard->issue_date ?? null)))
            <p class="issue_date"> {{ date('d-m-Y', strtotime($admit->issue_date ?? $admitCard->issue_date)) }}</p>
        @endif
    </div>
</body>
