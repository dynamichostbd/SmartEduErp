            ...
            <div class="grid grid-cols-1 gap-4 lg:grid-cols-12">
                ...
                <div class="lg:col-span-9">
                    <div class="rounded-sm border border-slate-300 bg-white">
                        <div class="border-b border-slate-300 bg-slate-50 px-4 py-3 text-sm font-extrabold text-slate-800">INSTITUTION PAID INVOICE</div>
                        <div class="p-4">
                            <div v-if="studentDashboardLoading" class="text-sm text-slate-600">Loading...</div>
                            <div v-else-if="studentDashboardError" class="rounded-sm border border-red-200 bg-red-50 p-3 text-sm text-red-800">{{ studentDashboardError }}</div>
                            <div v-else>
                                    ...
                                    <table class="min-w-full text-sm">
                                        ...
                                        <tbody>
                                            <tr v-for="(inv, idx) in studentDashboardInvoices" :key="'sdinv-' + inv.id" class="border-b last:border-b-0">
                                                ...
                                                <td class="px-3 py-2">
                                                    <button type="button" class="rounded-sm bg-[#2b6cb0] px-3 py-1.5 text-xs font-extrabold text-white hover:bg-[#245a94]" @click="studentDashboardViewInvoice(inv)">
                                                        VIEW INVOICE
                                                    </button>
                                                </td>
                                            </tr>
                                ...
                                <div v-else class="rounded-sm border border-slate-300 bg-slate-50 p-4 text-center text-sm text-slate-700">No paid invoices found.</div>
                            </div>
                        </div>
                    </div>

                    <div class="mt-4 rounded-sm border border-slate-300 bg-white">
                        <div class="border-b border-slate-300 bg-slate-50 px-4 py-3 text-sm font-extrabold text-slate-800">RECENT NOTICE</div>
                        <div class="p-4">
                                ...
                                <table class="min-w-full text-sm">
                                    ...
                                    <tbody>
                                        <tr v-for="(n, idx) in studentDashboardRecentNotices" :key="'sdn-' + n.id" class="border-b last:border-b-0">
                                            ...
                                            <td class="px-3 py-2">
                                                <button type="button" class="rounded-sm border border-slate-300 bg-white px-2 py-1 text-xs font-extrabold text-slate-700 hover:bg-slate-50" @click="go('/notices/' + n.id)">View</button>
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
                                <div v-if="!studentDashboardRecentNotices.length" class="mt-3 rounded-sm border border-slate-300 bg-slate-50 p-4 text-center text-sm text-slate-700">No notices found.</div>
                            </div>
                        </div>
                    </div>
                </div>
