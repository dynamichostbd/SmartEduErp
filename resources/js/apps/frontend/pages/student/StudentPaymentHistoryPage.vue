            ...
            <div class="grid grid-cols-1 gap-4 lg:grid-cols-12">
                ...
                <div class="lg:col-span-9">
                    <div class="rounded-md border border-slate-200 bg-white">
                        <div class="border-b border-slate-200 bg-slate-50 px-4 py-3 text-sm font-extrabold text-slate-800">PAYMENT HISTORY</div>
                        <div class="p-4">
                            ...
                            <div v-else>
                                <div v-if="invoices.length" class="overflow-x-auto">
                                    <table class="min-w-full text-sm">
                                        ...
                                        <tbody>
                                            <tr v-for="(inv, idx) in invoices" :key="'sph-' + inv.id" class="border-b last:border-b-0">
                                                ...
                                                <td class="px-3 py-2">
                                                    <span
                                                        class="rounded-sm px-2 py-1 text-xs font-extrabold"
                                                ...
                                                <td class="px-3 py-2">
                                                    <button type="button" class="rounded-md bg-[#2b6cb0] px-3 py-1.5 text-xs font-extrabold text-white hover:bg-[#245a94]" @click="viewInvoice(inv)">
                                                        VIEW INVOICE
                                                    </button>
                                                </td>
                                            </tr>
                                ...
                                <div v-else class="rounded-md border border-slate-200 bg-slate-50 p-4 text-center text-sm text-slate-700">No payment history found.</div>
                            </div>
                        </div>
                    </div>
                </div>
