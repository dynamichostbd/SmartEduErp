                            ...
                            <div v-else>
                                <div v-if="cards.length" class="overflow-x-auto">
                                    <table class="min-w-full text-sm">
                                        ...
                                        <tbody>
                                            <tr v-for="(c, idx) in cards" :key="'sac-' + c.id"
                                                ...
                                                <td class="px-3 py-2">
                                                    <span class="rounded-sm px-2 py-1 text-xs font-bold"
                                                        :class="eligible(c) ? 'bg-emerald-100 text-emerald-800' : 'bg-red-100 text-red-800'">
                                                        {{ eligible(c) ? 'ELIGIBLE' : 'NOT ELIGIBLE' }}
                                                    </span>
                                                </td>
                                                <td class="px-3 py-2">
                                                    <button type="button"
                                                        class="rounded-sm bg-[#0b1d4d] px-3 py-1.5 text-xs font-bold text-white hover:bg-[#09163c] disabled:cursor-not-allowed disabled:opacity-60"
                                                        :disabled="!eligible(c)" @click="download(c)">
                                                        Download
                                                    </button>
                                                </td>
                                            </tr>
