    ...
    <div class="flex flex-col gap-4">
        <div class=" border border-slate-300 bg-white p-5">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                <div>
                    <div class="text-xl font-semibold text-slate-900">Role List</div>
                </div>
                ...
                <table class="min-w-full border-collapse border border-slate-300">
                    <thead class="bg-emerald-100">
                        <tr class="text-left text-xs font-semibold uppercase tracking-wider text-slate-700">
                            ...
                            <th class="border border-slate-300 bg-emerald-100 px-1 py-2 text-slate-700">Role Type</th>
                    ...
                    <tbody>
                        ...
                        >
                            ...
                            <td class="border border-slate-300 px-1 py-1 font-medium text-slate-900">{{ row.name || '' }}</td>
                            <td class="border border-slate-300 px-1 py-1 text-center">
                                <span class="inline-flex rounded-full px-1 py-0.5 text-xs font-semibold" :class="String(row.status) === 'active' ? 'bg-emerald-50 text-emerald-700' : 'bg-rose-50 text-rose-700'">
                                    {{ row.status || '' }}
                                </span>
                            </td>
...
export default {
    ...
    computed: {
        ...
        canCreate() {
            return !this.perms.length || this.perms.includes('role.create')
        },
        canEdit() {
            return !this.perms.length || this.perms.includes('role.edit')
        },
        canDelete() {
            return !this.perms.length || this.perms.includes('role.destroy')
        },
        paginationPages() {
    ...
    methods: {
        ...
        goCreate() {
            this.go('/admin/role/create')
        },
        goEdit(id) {
            this.go(`/admin/role/${id}/edit`)
        },
        goView(id) {
            this.go(`/admin/role/${id}`)
        },
        reset() {
        ...
        async load(page = 1) {
            ...
            try {
                const res = await window.axios.get('/admin/role', {
                    params: {
                        page,
                ...
                this.error = e?.response?.data?.message || 'Failed to load roles.'
            } finally {
                this.loading = false
            }
        },
        async destroy(id) {
            ...
            this.deletingId = id
            try {
                const res = await window.axios.delete(`/admin/role/${id}`)
