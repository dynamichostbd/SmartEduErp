    ...
    <div class="flex flex-col gap-4">
        <div class="rounded-2xl border border-slate-200 bg-white p-5">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                <div>
                    <div class="text-xl font-semibold text-slate-900">Account Head</div>
                </div>
                ...
                <div class="flex w-full flex-col gap-2 sm:w-auto sm:flex-row">
                    ...
                    <select v-model="filters.type" class="h-10 rounded-lg border border-slate-200 bg-white px-3 text-sm outline-none" :disabled="loading" @change="load(1)">
                ...
                <table class="min-w-full border-collapse border border-slate-200">
                    <thead class="bg-slate-50">
                        <tr class="text-left text-xs font-semibold uppercase tracking-wider text-slate-500">
                            ...
                            <th class="border border-slate-200 bg-slate-50 px-4 py-3 text-center">Type</th>
                            <th class="border border-slate-200 bg-slate-50 px-4 py-3 text-center">Status</th>
                            <th class="border border-slate-200 bg-slate-50 px-4 py-3">Created at</th>
                    ...
                    <tbody>
                        ...
                        <tr v-for="(row, idx) in rows" v-else :key="row.id" class="text-sm text-slate-800" :class="idx % 2 === 0 ? 'bg-white' : 'bg-slate-50'">
                            ...
                            <td class="border border-slate-200 px-4 py-3">{{ row.name_en || '—' }}</td>
                            <td class="border border-slate-200 px-4 py-3 text-center">{{ row.type || '—' }}</td>
                            <td class="border border-slate-200 px-4 py-3 text-center">{{ row.status || '—' }}</td>
                            <td class="border border-slate-200 px-4 py-3">{{ row.created_at || '—' }}</td>
...
export default {
    ...
    methods: {
        ...
        goCreate() {
            this.go('/admin/accountHead/create')
        },
        goView(id) {
            this.go(`/admin/accountHead/${id}`)
        },
        goEdit(id) {
        ...
        async load(page = 1) {
            ...
            this.error = ''
            try {
                const res = await window.axios.get('/admin/accountHead', {
                    params: {
                        page,
                ...
                this.error = e?.response?.data?.message || 'Failed to load account heads.'
            } finally {
                this.loading = false
            }
        },
        async destroy(id) {
            ...
            this.deletingId = id
            try {
                const res = await window.axios.delete(`/admin/accountHead/${id}`)
