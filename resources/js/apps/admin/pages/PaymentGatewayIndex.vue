        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-5">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                ...
                <div class="flex w-full flex-col gap-2 sm:w-auto sm:flex-row">
                    <select v-model="filters.status" class="h-10 rounded-lg border border-slate-200 bg-white px-3 text-sm outline-none" :disabled="loading" @change="load(1)">
                ...
                <table class="min-w-full border-collapse border border-slate-200">
                    <thead class="bg-slate-50">
                        <tr class="text-left text-xs font-semibold uppercase tracking-wider text-slate-500">
                            ...
                            <th class="border border-slate-200 bg-slate-50 px-4 py-3">Department/Group</th>
                            <th class="border border-slate-200 bg-slate-50 px-4 py-3">Title</th>
                            <th class="border border-slate-200 bg-slate-50 px-4 py-3 text-center">Provider</th>
                            <th class="border border-slate-200 bg-slate-50 px-4 py-3">Store ID</th>
                            <th class="border border-slate-200 bg-slate-50 px-4 py-3">Store Password</th>
                            <th class="border border-slate-200 bg-slate-50 px-4 py-3">Account No</th>
                    ...
                    <tbody>
                        ...
                        <tr v-for="(row, idx) in rows" v-else :key="row.id" class="text-sm text-slate-800" :class="idx % 2 === 0 ? 'bg-white' : 'bg-slate-50'">
                            ...
                            <td class="border border-slate-200 px-4 py-3">{{ deptName(row.department_id) }}</td>
                            ...
                            <td class="border border-slate-200 px-4 py-3 text-center">{{ row.provider || '—' }}</td>
                            <td class="border border-slate-200 px-4 py-3">{{ row.store_id || '—' }}</td>
                            <td class="border border-slate-200 px-4 py-3">{{ row.store_password || '—' }}</td>
                            <td class="border border-slate-200 px-4 py-3">{{ row.account_no || '—' }}</td>
                            <td class="border border-slate-200 px-4 py-3 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    ...
                                    <button type="button" class="inline-flex h-8 w-8 items-center justify-center rounded-md border border-slate-200 bg-white text-slate-700 hover:bg-slate-50" title="Edit" @click="goEdit(row.id)">
                                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="h-4 w-4">
