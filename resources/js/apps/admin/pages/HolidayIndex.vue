    ...
    <div class="flex flex-col gap-4">
        <div class="rounded-2xl border border-slate-200 bg-white p-5">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                <div>
                    <div class="text-xl font-semibold text-slate-900">Holiday</div>
                </div>
                ...
                <div class="flex w-full flex-col gap-2 sm:w-auto sm:flex-row">
                    <select v-model="filters.field_name" class="h-9 rounded-lg border border-slate-200 bg-white px-3 text-sm outline-none focus:border-slate-300 focus:ring-2 focus:ring-slate-200" :disabled="loading">
                        ...
                        <option value="holiday_type">Holiday Type</option>
                    </select>
                ...
                <table class="min-w-full border-collapse border border-slate-200">
                    <thead class="bg-slate-50">
                        <tr class="text-left text-xs font-semibold uppercase tracking-wider text-slate-500">
                            ...
                            <th class="border border-slate-200 bg-slate-50 px-4 py-3">Holiday Type</th>
                            <th class="border border-slate-200 bg-slate-50 px-4 py-3">Holiday Name</th>
                            <th class="border border-slate-200 bg-slate-50 px-4 py-3 text-center">From Date</th>
                            <th class="border border-slate-200 bg-slate-50 px-4 py-3 text-center">To Date</th>
                            <th class="border border-slate-200 bg-slate-50 px-4 py-3 text-center">Total Days</th>
                    ...
                    <tbody>
                        <tr v-if="loading" class="text-sm text-slate-600">
                            <td class="border border-slate-200 px-4 py-6" colspan="8">Loading...</td>
                        </tr>
                        <tr v-else-if="rows.length === 0" class="text-sm text-slate-600">
                            <td class="border border-slate-200 px-4 py-6" colspan="8">No data found.</td>
                        </tr>
                        <tr v-for="(row, idx) in rows" v-else :key="row.id" class="text-sm text-slate-800" :class="idx % 2 === 0 ? 'bg-white' : 'bg-slate-50'">
                            ...
                            <td class="border border-slate-200 px-4 py-3">{{ row.holiday_type || '—' }}</td>
                            ...
                            <td class="border border-slate-200 px-4 py-3 text-center">{{ row.from_date || '—' }}</td>
                            <td class="border border-slate-200 px-4 py-3 text-center">{{ row.to_date || '—' }}</td>
                            <td class="border border-slate-200 px-4 py-3 text-center">{{ row.total_days ?? '—' }}</td>
...
export default {
    ...
    computed: {
        ...
        canCreate() {
            return !this.perms.length || this.perms.includes('holiday.create')
        },
        canEdit() {
            return !this.perms.length || this.perms.includes('holiday.edit')
        },
        canDelete() {
            return !this.perms.length || this.perms.includes('holiday.destroy')
        },
    },
    created() {
        this.load(1)
    ...
    methods: {
        ...
        goCreate() {
            this.go('/admin/holiday/create')
        },
        goView(id) {
            this.go(`/admin/holiday/${id}`)
        },
        goEdit(id) {
        ...
        async load(page = 1) {
            ...
            this.error = ''
            try {
                const res = await window.axios.get('/admin/holiday', {
                    params: {
                        page,
        ...
        async destroy(id) {
            ...
            this.deletingId = id
            try {
                const res = await window.axios.delete(`/admin/holiday/${id}`)
