    ...
    <div class="flex flex-col gap-4">
        <div class="rounded-2xl border border-slate-200 bg-white p-5">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div class="min-w-0">
                    <div class="truncate text-xl font-semibold text-slate-900">Attendance</div>
                    <div class="mt-1 text-sm text-slate-600">Daily attendance list</div>
                </div>
                ...
                <table class="min-w-full divide-y divide-slate-200 text-sm">
                    ...
                    <tbody class="divide-y divide-slate-200 bg-white">
                        <tr v-for="row in rows" :key="'a-' + row.id">
                            <td class="px-3 py-2 text-slate-800">{{ row.date || '--' }}</td>
                            ...
                            <td class="px-3 py-2 text-center text-slate-800">{{ row.total_student ?? 0 }}</td>
                            <td class="px-3 py-2 text-center text-emerald-700">{{ row.present_student_count ?? 0 }}</td>
                            <td class="px-3 py-2 text-center text-rose-700">{{ Math.max(0, Number(row.total_student ?? 0) - Number(row.present_student_count ?? 0)) }}</td>
...
export default {
    ...
    methods: {
        ...
        async search(resetPage) {
            ...
            this.error = ''
            try {
                const res = await window.axios.get('/admin/attendance/list', { params: this.filters })
        ...
        async destroyRow(id) {
            ...
            this.error = ''
            try {
                await window.axios.delete(`/admin/attendance/${id}`)
