    ...
    <div class="flex flex-col gap-4">
        <div class=" border border-slate-300 bg-white p-5">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                <div>
                    <div class="text-xl font-semibold text-slate-900">Subject Assign</div>
                </div>
            ...
            <div class="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-3">
                <select v-model="filters.academic_qualification_id" class="h-9 rounded-sm border border-slate-300 bg-white px-3 text-sm outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100" :disabled="loading">
                    <option value="">All Academic Levels</option>
                    <option v-for="q in qualifications" :key="'q-' + q.id" :value="String(q.id)">{{ q.name }}</option>
                </select>

                <select v-model="filters.department_id" class="h-9 rounded-sm border border-slate-300 bg-white px-3 text-sm outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100" :disabled="loading || !filters.academic_qualification_id">
                    <option value="">All Departments</option>
                    <option v-for="d in departmentsFiltered" :key="'d-' + d.id" :value="String(d.id)">{{ d.name }}</option>
                </select>

                <select v-model="filters.academic_class_id" class="h-9 rounded-sm border border-slate-300 bg-white px-3 text-sm outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100" :disabled="loading || !filters.academic_qualification_id">
                ...
                <table class="min-w-full border-collapse border border-slate-300">
                    <thead class="bg-emerald-100">
                        <tr class="text-left text-xs font-semibold uppercase tracking-wider text-slate-700">
                            ...
                            <th class="border border-slate-300 bg-emerald-100 px-1 py-2 text-slate-700">Main Subject</th>
                            <th class="border border-slate-300 bg-emerald-100 px-1 py-2 text-slate-700">Note</th>
                    ...
                    <tbody>
                        <tr v-if="loading" class="text-sm text-slate-600">
                            <td class="border border-slate-300 px-4 py-6" colspan="7">Loading...</td>
                        </tr>
                        <tr v-else-if="rows.length === 0" class="text-sm text-slate-600">
                            <td class="border border-slate-300 px-4 py-6" colspan="7">No data found.</td>
                        </tr>
                        <tr v-for="(row, idx) in rows" :key="row.id" :class="idx % 2 === 0 ? 'bg-white' : 'bg-slate-50'">
                            ...
                            <td class="border border-slate-300 px-1 py-1 text-slate-800">{{ row.qualification_name || '—' }}</td>
                            <td class="border border-slate-300 px-1 py-1 text-slate-800">{{ row.department_name || '—' }}</td>
                            <td class="border border-slate-300 px-1 py-1 text-slate-800">{{ row.class_name || '—' }}</td>
                            <td class="border border-slate-300 px-1 py-1 text-slate-800">{{ row.main_subject ?? '—' }}</td>
                            <td class="border border-slate-300 px-1 py-1 text-slate-800">{{ row.note || '—' }}</td>
...
export default {
    ...
    computed: {
        ...
        canCreate() {
            return !this.perms.length || this.perms.includes('subjectAssign.create')
        },
        canEdit() {
            return !this.perms.length || this.perms.includes('subjectAssign.edit')
        },
        canDelete() {
            return !this.perms.length || this.perms.includes('subjectAssign.destroy')
        },
        qualifications() {
        ...
        departmentsFiltered() {
            const qid = String(this.filters.academic_qualification_id || '')
            if (!qid) return []
    ...
    methods: {
        ...
        goCreate() {
            this.go('/admin/subjectAssign/create')
        },
        goView(id) {
            this.go(`/admin/subjectAssign/${id}`)
        },
        goEdit(id) {
        ...
        async load(page = 1) {
            ...
            this.error = ''
            try {
                const res = await window.axios.get('/admin/subjectAssign', {
                    params: {
                        page,
                ...
                this.error = e?.response?.data?.message || 'Failed to load subject assigns.'
            } finally {
                this.loading = false
            }
        },
        async destroy(id) {
            ...
            this.deletingId = id
            try {
                const res = await window.axios.delete(`/admin/subjectAssign/${id}`)
