    ...
    <div class="flex flex-col gap-4">
        <div class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div class="min-w-0">
                    ...
                    <div class="mt-1 text-sm text-slate-600">{{ isEdit ? 'Update department information' : 'Create a new department' }}</div>
                </div>
        ...
        <div class="grid grid-cols-1 gap-4 lg:grid-cols-12">
            <div class="space-y-4 lg:col-span-8 xl:col-span-9">
                <div class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
                    <div class="min-w-0">
                        <div class="text-sm font-semibold text-slate-900">Department Details</div>
                        <div class="mt-1 text-xs text-slate-500">Basic information and settings</div>
                    </div>
                    ...
                    <div class="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                        ...
                        <div>
                            ...
                            <select v-model="form.application_fee" class="mt-1 h-9 w-full rounded-sm border border-slate-300 bg-white px-3 text-sm shadow-sm outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100">
                                <option :value="1">Yes</option>
                                <option :value="0">No</option>
                            </select>
                        </div>
                        ...
                        <div class="sm:col-span-2 lg:col-span-3">
                            ...
                            <textarea v-model="form.description" rows="4" class="mt-1 w-full rounded-sm border border-slate-300 bg-white px-3 py-2 text-sm shadow-sm outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100"></textarea>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="max-h-[420px] overflow-y-auto p-5">
                        ...
                        <div v-else class="space-y-2">
                            <div v-for="q in deptQuals" :key="'dq-' + q.academic_qualification_id" class="grid grid-cols-12 items-center gap-3 rounded-xl border border-slate-200 bg-white p-3">
                                ...
                                <input v-model="q.department_code" type="text" class="col-span-5 h-9 rounded-sm border border-slate-300 bg-white px-3 text-sm text-center shadow-sm outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100" placeholder="Department Code" :disabled="!q.checked" />
                            </div>
                        </div>
                    </div>
                </div>
