    ...
    <div class="flex flex-col gap-4">
        <div class=" border border-slate-300 bg-white p-5">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                <div>
                    <div class="text-xl font-semibold text-slate-900">Academic Level</div>
                </div>
                ...
                <table class="min-w-full border-collapse border border-slate-300">
                    <thead class="bg-emerald-100">
                        <tr class="text-left text-xs font-semibold uppercase tracking-wider text-slate-700">
                            ...
                            <th class="border border-slate-300 bg-emerald-100 px-1 py-2 text-slate-700">Qualification Name</th>
                            <th class="border border-slate-300 bg-emerald-100 px-1 py-2 text-center text-slate-700">Registration</th>
                            <th class="border border-slate-300 bg-emerald-100 px-1 py-2 text-center text-slate-700">Online Admission</th>
                            <th class="border border-slate-300 bg-emerald-100 px-1 py-2 text-center text-slate-700">Check Application Fees</th>
                            <th class="border border-slate-300 bg-emerald-100 px-1 py-2 text-center text-slate-700">Admission Roll Verify</th>
                            <th class="border border-slate-300 bg-emerald-100 px-1 py-2 text-center text-slate-700">Subject Choose</th>
                            <th class="border border-slate-300 bg-emerald-100 px-1 py-2 text-center text-slate-700">Migration</th>
                            <th class="border border-slate-300 bg-emerald-100 px-1 py-2 text-center text-slate-700">Application Fee</th>
                    ...
                    <tbody>
                        ...
                        <tr v-for="(row, idx) in rows" v-else :key="row.id" class="text-sm text-slate-800" :class="idx % 2 === 0 ? 'bg-white' : 'bg-slate-50'">
                            ...
                            <td class="border border-slate-300 px-1 py-1 text-center">{{ yesNo(row.application_fees) }}</td>
                            <td class="border border-slate-300 px-1 py-1 text-center">{{ yesNo(row.admission_roll_verify) }}</td>
                            <td class="border border-slate-300 px-1 py-1 text-center">{{ yesNo(row.subject_choose) }}</td>
                            <td class="border border-slate-300 px-1 py-1 text-center">{{ yesNo(row.migration) }}</td>
...
<script>
export default {
    name: 'AcademicQualificationIndex',
    props: {
        systems: {
            type: Object,
            default: () => ({}),
    ...
    computed: {
        ...
        canCreate() {
            return !this.perms.length || this.perms.includes('academicQualification.create')
        },
        canEdit() {
            return !this.perms.length || this.perms.includes('academicQualification.edit')
        },
        canDelete() {
            return !this.perms.length || this.perms.includes('academicQualification.destroy')
        },
    },
    created() {
        this.load(1)
    ...
    methods: {
        ...
        goCreate() {
            this.go('/admin/academicQualification/create')
        },
        goView(id) {
            this.go(`/admin/academicQualification/${id}`)
        },
        goEdit(id) {
        ...
        async load(page = 1) {
            ...
            this.error = ''
            try {
                const res = await window.axios.get('/admin/academicQualification', {
                    params: {
                        page,
                ...
                this.error = e?.response?.data?.message || 'Failed to load academic levels.'
            } finally {
                this.loading = false
            }
        },
        async destroy(id) {
            ...
            this.deletingId = id
            try {
                const res = await window.axios.delete(`/admin/academicQualification/${id}`)
