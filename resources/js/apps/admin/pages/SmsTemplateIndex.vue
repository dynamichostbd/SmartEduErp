    ...
    <div class="flex flex-col gap-4">
        <div class=" border border-slate-300 bg-white p-5">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                <div>
                    <div class="text-xl font-semibold text-slate-900">SMS Template</div>
                </div>
                ...
                <table class="min-w-full border-collapse border border-slate-300">
                    <thead class="bg-emerald-100">
                        <tr class="text-left text-xs font-semibold uppercase tracking-wider text-slate-700">
                            ...
                            <th class="border border-slate-300 bg-emerald-100 px-1 py-2 text-slate-700">Template Name</th>
                            <th class="border border-slate-300 bg-emerald-100 px-1 py-2 text-slate-700">SMS Type</th>
                            <th class="border border-slate-300 bg-emerald-100 px-1 py-2 text-center text-slate-700">Common Message</th>
                            <th class="border border-slate-300 bg-emerald-100 px-1 py-2 text-center text-slate-700">Sending Status</th>
                    ...
                    <tbody>
                        ...
                        <tr v-for="(row, idx) in rows" v-else :key="row.id" class="text-sm text-slate-800" :class="idx % 2 === 0 ? 'bg-white' : 'bg-slate-50'">
                            ...
                            <td class="border border-slate-300 px-1 py-1">{{ smsTypeLabel(row.sms_type) }}</td>
                            <td class="border border-slate-300 px-1 py-1 text-center">{{ Number(row.common_message || 0) === 1 ? 'YES' : 'NO' }}</td>
                            <td class="border border-slate-300 px-1 py-1 text-center">{{ Number(row.sending_status || 0) === 1 ? 'ONLINE' : 'OFFLINE' }}</td>
...
export default {
    ...
    computed: {
        ...
        canCreate() {
            return !this.perms.length || this.perms.includes('smsTemplate.create')
        },
        canEdit() {
            return !this.perms.length || this.perms.includes('smsTemplate.edit')
        },
        canDelete() {
            return !this.perms.length || this.perms.includes('smsTemplate.destroy')
        },
        smsTypes() {
    ...
    methods: {
        ...
        goCreate() {
            this.go('/admin/smsTemplate/create')
        },
        goView(id) {
            this.go(`/admin/smsTemplate/${id}`)
        },
        goEdit(id) {
        ...
        async load(page = 1) {
            ...
            this.error = ''
            try {
                const res = await window.axios.get('/admin/smsTemplate', {
                    params: {
                        page,
                ...
                this.error = e?.response?.data?.message || 'Failed to load SMS templates.'
            } finally {
                this.loading = false
            }
        },
        async destroy(id) {
            ...
            this.deletingId = id
            try {
                const res = await window.axios.delete(`/admin/smsTemplate/${id}`)
