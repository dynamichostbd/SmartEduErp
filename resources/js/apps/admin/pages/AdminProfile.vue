        ...
        <div class=" border border-slate-300 bg-white p-5">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div class="flex flex-wrap items-center gap-2">
                    ...
                    <button type="button" class="rounded-sm bg-emerald-600 px-4 py-2 text-sm font-semibold text-white hover:bg-emerald-700" :disabled="saving" @click="saveProfile">{{ saving ? '...' : 'Update Information' }}</button>
                </div>
            </div>
        </div>
            ...
            <div class="lg:col-span-4">
                <div class=" border border-slate-300 bg-white p-5">
                    <div class="flex flex-col items-center text-center">
                        <img
                            :src="previewProfile || admin?.profile || fallback"
                            alt="Admin"
                            class="h-28 w-28 rounded-full border border-slate-300 object-cover"
                        />
                        <div class="mt-3">
                    ...
                    <div class="mt-4">
                        <div class="overflow-hidden rounded-xl border border-slate-300">
                            ...
                            <div class="flex items-center justify-between border-t border-slate-300 px-3 py-2 text-sm">
            ...
            <div class="lg:col-span-8">
                <div class=" border border-slate-300 bg-white p-5">
                    <div class="flex flex-wrap gap-2 border-b border-slate-300 pb-3">
                        <button type="button" class="rounded-sm px-3 py-2 text-sm font-semibold" :class="tab === 'edit' ? 'bg-emerald-600 text-white' : 'bg-slate-100 text-slate-700'" @click="tab = 'edit'">Profile Edit</button>
                        <button type="button" class="rounded-sm px-3 py-2 text-sm font-semibold" :class="tab === 'password' ? 'bg-emerald-600 text-white' : 'bg-slate-100 text-slate-700'" @click="tab = 'password'">Change Password</button>
                    </div>
                    ...
                    <div v-if="tab === 'edit'" class="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-12">
                        ...
                        <div class="lg:col-span-6">
                            ...
                            <input v-model="form.email" type="email" class="mt-1 h-9 w-full rounded-sm border border-slate-300 bg-white px-3 text-sm outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100" />
                        </div>
                        <div class="lg:col-span-6">
                        ...
                        <div class="lg:col-span-6">
                            ...
                            <input v-model="form.emergency_contacts" type="text" class="mt-1 h-9 w-full rounded-sm border border-slate-300 bg-white px-3 text-sm outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100" />
                        </div>
                    </div>
                    ...
                    <div v-else class="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-12">
                        <div class="lg:col-span-6">
                            ...
                            <input v-model="pass.old_password" type="password" class="mt-1 h-9 w-full rounded-sm border border-slate-300 bg-white px-3 text-sm outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100" @blur="checkOld" />
                        ...
                        <div class="lg:col-span-6">
                            ...
                            <input v-model="pass.new_password" type="password" class="mt-1 h-9 w-full rounded-sm border border-slate-300 bg-white px-3 text-sm outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100" />
                        </div>
                        <div class="lg:col-span-6">
                            <div class="text-xs font-semibold text-slate-600">Confirm Password</div>
                            <input v-model="pass.confirm_password" type="password" class="mt-1 h-9 w-full rounded-sm border border-slate-300 bg-white px-3 text-sm outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100" />
                        </div>

                        <div class="lg:col-span-12">
                            <button type="button" class="rounded-sm bg-slate-900 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-800" :disabled="savingPassword" @click="changePassword">
...
export default {
    ...
    methods: {
        ...
        async load() {
            ...
            this.loading = true
            try {
                const res = await window.axios.get(`/admin/admin/${id}`)
        ...
        async saveProfile() {
            ...
            try {
                ...
                const res = await window.axios.post(`/admin/admin/${id}`, fd, {
                    headers: { 'Content-Type': 'multipart/form-data' },
                })

                this.success = res?.data?.message || 'Update Successfully!'
        ...
        async checkOld() {
            ...
            try {
                const res = await window.axios.post('/admin/check-old-password', { old_password: old })
