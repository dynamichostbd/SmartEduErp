    ...
    <div class="flex flex-col gap-4">
        <div class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div class="min-w-0">
                    <div class="truncate text-xl font-semibold text-slate-900">{{ isEdit ? 'Edit Designation' : 'Create Designation' }}</div>
                    <div class="mt-1 text-sm text-slate-600">{{ isEdit ? 'Update designation information' : 'Create a new designation' }}</div>
                </div>
        ...
        <div class="grid grid-cols-1 gap-4 lg:grid-cols-12">
            <div class="space-y-4 lg:col-span-8 xl:col-span-9">
                <div class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
                    <div class="min-w-0">
                        <div class="text-sm font-semibold text-slate-900">Designation Details</div>
                        <div class="mt-1 text-xs text-slate-500">Name and short name</div>
                    </div>
                    ...
                    <div class="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                        ...
                        <div>
                            ...
                            <input v-model="form.short_name" type="text" class="mt-1 h-9 w-full rounded-sm border border-slate-300 bg-white px-3 text-sm shadow-sm outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100" />
                        </div>
                    </div>
                </div>
            </div>
...
export default {
    ...
    methods: {
        ...
        goIndex() {
            this.go('/admin/designation')
        },
        async load() {
            this.error = ''
            try {
                const res = await window.axios.get(`/admin/designation/${this.designationId}`)
                const row = res?.data?.designation || res?.data?.data || res?.data || {}
                this.form.name = row?.name || ''
                this.form.short_name = row?.short_name || ''
                this.form.status = row?.status || 'active'
            } catch (e) {
                this.error = e?.response?.data?.message || 'Failed to load designation.'
            }
        },
        async submit() {
            this.saving = true
            ...
            try {
                ...
                let res
                if (this.isEdit) {
                    res = await window.axios.put(`/admin/designation/${this.designationId}`, payload)
                } else {
                    res = await window.axios.post('/admin/designation', payload)
                }
                ...
                if (!this.isEdit) {
                    ...
                    if (id) {
                        this.go(`/admin/designation/${id}/edit`)
                    } else {
                        this.goIndex()
                    }
                }
