    ...
    <div class="flex flex-col gap-4">
        <div class=" border border-slate-300 bg-white p-5">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                <div>
                    <div class="text-xl font-semibold text-slate-900">Popup List</div>
                </div>
                ...
                <div class="flex w-full flex-col gap-2 sm:w-auto sm:flex-row">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <button v-if="canCreate" class="h-9 rounded-sm bg-slate-900 px-4 text-sm font-semibold text-white hover:bg-slate-800" :disabled="loading" @click="goCreate">Add Popup</button>
                ...
                <table class="min-w-full border-collapse border border-slate-300">
                    <thead class="bg-emerald-100">
                        <tr class="text-left text-xs font-semibold uppercase tracking-wider text-slate-700">
                            ...
                            <th class="border border-slate-300 bg-emerald-100 px-1 py-2 text-center text-slate-700">Image</th>
                    ...
                    <tbody>
                        ...
                        <tr v-for="(row, idx) in rows" v-else :key="row.id" class="text-sm text-slate-800" :class="idx % 2 === 0 ? 'bg-white' : 'bg-slate-50'">
                            ...
                            <td class="border border-slate-300 px-1 py-1 text-center">
                                <a v-if="row.image" class="text-emerald-700 hover:underline" :href="resolveImage(row.image)" target="_blank">View</a>
                                <span v-else>—</span>
                            </td>
...
export default {
    ...
    computed: {
        ...
        canCreate() {
            return !this.perms.length || this.perms.includes('popup.create')
        },
        canEdit() {
            return !this.perms.length || this.perms.includes('popup.edit')
        },
        canDelete() {
            return !this.perms.length || this.perms.includes('popup.destroy')
        },
    },
    created() {
        this.load(1)
    ...
    methods: {
        ...
        goCreate() {
            this.go('/admin/popup/create')
        },
        goView(id) {
            this.go(`/admin/popup/${id}`)
        },
        goEdit(id) {
            this.go(`/admin/popup/${id}/edit`)
        },
        reset() {
        ...
        async load(page = 1) {
            ...
            this.error = ''
            try {
                const res = await window.axios.get('/admin/popup', {
                    params: {
                        page,
                ...
                this.error = e?.response?.data?.message || 'Failed to load popups.'
            } finally {
                this.loading = false
            }
        },
        async destroy(id) {
            ...
            this.deletingId = id
            try {
                const res = await window.axios.delete(`/admin/popup/${id}`)
