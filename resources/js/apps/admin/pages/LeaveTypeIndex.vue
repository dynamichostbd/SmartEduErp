    ...
    <div class="flex flex-col gap-4">
        <div class=" border border-slate-300 bg-white p-5">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                <div>
                    <div class="text-xl font-semibold text-slate-900">Leave Type</div>
                </div>
                ...
                <table class="min-w-full border-collapse border border-slate-300">
                    <thead class="bg-emerald-100">
                        <tr class="text-left text-xs font-semibold uppercase tracking-wider text-slate-700">
                            ...
                            <th class="border border-slate-300 bg-emerald-100 px-1 py-2">Name</th>
                            <th class="border border-slate-300 bg-emerald-100 px-1 py-2">Short Name</th>
                            <th class="border border-slate-300 bg-emerald-100 px-1 py-2 text-center">Allocated Days</th>
                            <th class="border border-slate-300 bg-emerald-100 px-1 py-2 text-center">Back Date</th>
                            <th class="border border-slate-300 bg-emerald-100 px-1 py-2 text-center">Documents</th>
                            <th class="border border-slate-300 bg-emerald-100 px-1 py-2 text-center">Reason</th>
                    ...
                    <tbody>
                        ...
                        <tr v-for="(row, idx) in rows" v-else :key="row.id" class="text-sm text-slate-800" :class="idx % 2 === 0 ? 'bg-white' : 'bg-slate-50'">
                            ...
                            <td class="border border-slate-300 px-1 py-1">{{ row.short_name || '—' }}</td>
                            <td class="border border-slate-300 px-1 py-1 text-center">{{ row.allocated_leave_days ?? '—' }}</td>
                            <td class="border border-slate-300 px-1 py-1 text-center">{{ yesNo(row.back_date_select) }}</td>
                            <td class="border border-slate-300 px-1 py-1 text-center">{{ yesNo(row.documents_upload) }}</td>
                            <td class="border border-slate-300 px-1 py-1 text-center">{{ yesNo(row.reason) }}</td>
...
export default {
    ...
    computed: {
        ...
        canCreate() {
            return !this.perms.length || this.perms.includes('leaveType.create')
        },
        canEdit() {
            return !this.perms.length || this.perms.includes('leaveType.edit')
        },
        canDelete() {
            return !this.perms.length || this.perms.includes('leaveType.destroy')
        },
    },
    created() {
        this.load(1)
    ...
    methods: {
        ...
        goCreate() {
            this.go('/admin/leaveType/create')
        },
        goView(id) {
            this.go(`/admin/leaveType/${id}`)
        },
        goEdit(id) {
        ...
        async load(page = 1) {
            ...
            this.error = ''
            try {
                const res = await window.axios.get('/admin/leaveType', {
                    params: {
                        page,
                ...
                this.error = e?.response?.data?.message || 'Failed to load leave types.'
            } finally {
                this.loading = false
            }
        },
        async destroy(id) {
            ...
            this.deletingId = id
            try {
                const res = await window.axios.delete(`/admin/leaveType/${id}`)
