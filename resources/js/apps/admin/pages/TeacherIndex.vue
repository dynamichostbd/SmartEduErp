        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-5">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                ...
                <div class="flex w-full flex-col gap-2 sm:w-auto sm:flex-row">
                    ...
                    <select v-model="filters.department_id" class="h-9 rounded-lg border border-slate-200 bg-white px-3 text-sm outline-none focus:border-slate-300 focus:ring-2 focus:ring-slate-200" :disabled="loading" @change="load(1)">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <button v-if="canCreate" class="h-9 rounded-lg bg-slate-900 px-4 text-sm font-semibold text-white hover:bg-slate-800" :disabled="loading" @click="goCreate">Add Teacher</button>

                        <button v-if="canImport" class="h-9 rounded-lg border border-slate-200 bg-white px-4 text-sm font-semibold text-slate-700 hover:bg-slate-50" :disabled="loading" @click="goImport">Import</button>
                ...
                <table class="min-w-full border-collapse border border-slate-200">
                    ...
                    <tbody>
                        ...
                        <tr v-for="(row, idx) in rows" v-else :key="row.id" class="text-sm text-slate-800" :class="idx % 2 === 0 ? 'bg-white' : 'bg-slate-50'">
                            ...
                            <td class="border border-slate-200 px-4 py-3">{{ row.department?.name || '—' }}</td>
                            <td class="border border-slate-200 px-4 py-3">{{ row.mobile || '' }}</td>
                            <td class="border border-slate-200 px-4 py-3 text-center">
                                <span class="inline-flex rounded-full px-2 py-0.5 text-xs font-semibold" :class="row.status === 'active' ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-700'">{{ row.status || '—' }}</span>
                            </td>
                            <td class="border border-slate-200 px-4 py-3 text-center">
