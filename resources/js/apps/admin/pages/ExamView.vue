    ...
    <div class="flex flex-col gap-4">
        <div class="rounded-2xl border border-slate-200 bg-white p-5">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div class="min-w-0">
                    <div class="truncate text-xl font-semibold text-slate-900">Exam Details</div>
                </div>
                ...
                <div class="flex flex-wrap items-center gap-2">
                    ...
                    <button type="button" class="rounded-lg bg-slate-900 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-800" :disabled="loading || !examId" @click="goEdit">Edit</button>
                </div>
            </div>
        </div>
        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-5">
            ...
            <div v-else class="grid grid-cols-1 gap-3 sm:grid-cols-2">
                ...
                <div class="rounded-xl border border-slate-200 p-4">
                    <div class="text-xs font-semibold text-slate-500">Exam Type</div>
                    <div class="mt-1 text-sm font-semibold text-slate-900">{{ examTypeLabel(row.exam_type) }}</div>
                </div>
                <div class="rounded-xl border border-slate-200 p-4">
                    <div class="text-xs font-semibold text-slate-500">Class Test Exam Id</div>
                    <div class="mt-1 text-sm font-semibold text-slate-900">{{ row.class_test_exam_id ?? '—' }}</div>
                </div>
                <div class="rounded-xl border border-slate-200 p-4 sm:col-span-2">
...
export default {
    ...
    methods: {
        ...
        goEdit() {
            this.go(`/admin/exam/${this.examId}/edit`)
        },
        async load() {
            ...
            try {
                ...
                this.row = res?.data?.exam || res?.data || {}
            } catch (e) {
