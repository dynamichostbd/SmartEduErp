    ...
    <form class="flex flex-col gap-4" @submit.prevent="submit">
        ...
        <div class="flex flex-col gap-4">
            <div class="rounded-xl border border-slate-200 bg-white shadow-sm">
                <div class="border-b border-slate-100 bg-slate-50 px-4 py-3 text-sm font-extrabold uppercase text-slate-800">Search Students For Attendance</div>
                ...
                <div class="p-4 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-6">
                    ...
                    <div>
                        <div class="text-xs font-semibold text-slate-600">Select Session <span class="text-rose-600">*</span></div>
                        <select
                    ...
                    <div>
                        <div class="text-xs font-semibold text-slate-600">Department/Group <span class="text-rose-600">*</span></div>
                        <select
                    ...
                    <div class="flex items-end">
                        <button
                            ...
                            @click="loadStudents"
                        >
                            <span v-if="loadingStudents" class="h-4 w-4 rounded-full border-2 border-white/30 border-t-white animate-spin"></span>
            ...
            <div class="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden">
                <div class="flex flex-col gap-2 border-b border-slate-100 bg-slate-50 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
                    <div class="text-sm font-extrabold uppercase text-slate-800">Students</div>
                    <div class="text-sm text-slate-800">
                        <span class="font-extrabold">Total Student : {{ totals.total }}</span>
                        <span class="mx-2 text-slate-300">|</span>
                        <span class="font-extrabold text-emerald-600">Present : {{ totals.present }}</span>
                        <span class="mx-2 text-slate-300">|</span>
                        <span class="font-extrabold text-rose-600">Absent : {{ totals.absent }}</span>
                    </div>
                </div>

                <div class="p-4">
                    <div class="overflow-x-auto">
                        <table v-if="students.length" class="min-w-full text-sm border-collapse border border-slate-200">
                            <thead class="bg-white">
                                <tr>
                                    <th class="border border-slate-200 px-3 py-2 text-center font-bold text-slate-800">#</th>
                                    <th class="border border-slate-200 px-3 py-2 text-center font-bold text-slate-800">Profile</th>
                                    ...
                                    <th class="border border-slate-200 px-3 py-2 text-left font-bold text-slate-800">Admission ID</th>
                            ...
                            <tbody class="bg-white">
                                <tr v-for="(row, idx) in students" :key="'std-' + String(row.student_id || row.id) + '-' + idx" :class="(idx % 2 !== 0) ? 'bg-slate-50' : 'bg-white'" class="hover:bg-slate-100 transition-colors">
                                    ...
                                    <td class="border border-slate-200 px-3 py-2 text-center p-0">
                                        <div class="mx-auto flex h-[37px] w-9 items-center justify-center overflow-hidden">
                                            <img v-if="row.profile" :src="row.profile" class="h-full w-full object-cover" />
                                        </div>
                                    </td>
                                    <td class="border border-slate-200 px-3 py-2 text-slate-700">{{ row.student_id || '--' }}</td>
                                    <td class="border border-slate-200 px-3 py-2 text-slate-700">{{ row.name || '--' }}</td>
                                    <td class="border border-slate-200 px-3 py-2 text-slate-700">{{ row.mobile || '--' }}</td>
                                    <td class="border border-slate-200 px-3 py-2 text-slate-700">{{ row.admission_id || '--' }}</td>
                                    <td class="border border-slate-200 px-3 py-2 text-slate-700">{{ row.college_roll || '--' }}</td>
                                    <td class="border border-slate-200 px-3 py-2 text-center p-0">
                                        <div class="flex items-center justify-center h-[37px]">
                                            <button type="button" class="btn_choose_sent bg_btn_chose_1">
                                                <input
                                                    ...
                                                    value="A"
                                                    v-model="row.status"
                                                    :class="row.status === 'A' ? 'absent-radio' : ''"
                                                />A
                                            </button>
                                            <button type="button" class="btn_choose_sent bg_btn_chose_2">
                                                <input
                                                    ...
                                                    value="P"
                                                    v-model="row.status"
                                                    :class="row.status === 'P' ? 'present-radio' : ''"
                                                />P
                                            </button>
                                        </div>
                                    </td>
