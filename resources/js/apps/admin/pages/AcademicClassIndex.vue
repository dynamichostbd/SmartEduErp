    ...
    <div class="flex flex-col gap-4">
        <div class=" border border-slate-300 bg-white p-5">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                <div>
                    <div class="text-xl font-semibold text-slate-900">Academic Class</div>
                </div>
                ...
                <table class="min-w-full border-collapse border border-slate-300">
                    <thead class="bg-emerald-100">
                        <tr class="text-left text-xs font-semibold uppercase tracking-wider text-slate-700">
                            ...
                            <th class="border border-slate-300 bg-emerald-100 px-1 py-2 text-slate-700">Qualification</th>
                            <th class="border border-slate-300 bg-emerald-100 px-1 py-2 text-slate-700">Class Name</th>
                            <th class="border border-slate-300 bg-emerald-100 px-1 py-2 text-slate-700">Description</th>
                    ...
                    <tbody>
                        <tr v-if="loading" class="text-sm text-slate-600">
                            <td class="border border-slate-300 px-1 py-1" colspan="9">Loading...</td>
                        </tr>
                        <tr v-else-if="rows.length === 0" class="text-sm text-slate-600">
                            <td class="border border-slate-300 px-1 py-1" colspan="9">No data found.</td>
                        </tr>
                        <tr v-for="(row, idx) in rows" v-else :key="row.id" class="text-sm text-slate-800" :class="idx % 2 === 0 ? 'bg-white' : 'bg-slate-50'">
                            ...
                            <td class="border border-slate-300 px-1 py-1 text-slate-700">
...
export default {
    ...
    computed: {
        ...
        canCreate() {
            return !this.perms.length || this.perms.includes('academicClass.create')
        },
        canEdit() {
            return !this.perms.length || this.perms.includes('academicClass.edit')
        },
        canDelete() {
            return !this.perms.length || this.perms.includes('academicClass.destroy')
        },
        qualifications() {
    ...
    methods: {
        ...
        goCreate() {
            this.go('/admin/academicClass/create')
        },
        goView(id) {
            this.go(`/admin/academicClass/${id}`)
        },
        goEdit(id) {
        ...
        async load(page = 1) {
            ...
            this.error = ''
            try {
                const res = await window.axios.get('/admin/academicClass', {
                    params: {
                        page,
                ...
                this.error = e?.response?.data?.message || 'Failed to load academic classes.'
            } finally {
                this.loading = false
            }
        },
        async destroy(id) {
            ...
            this.deletingId = id
            try {
                const res = await window.axios.delete(`/admin/academicClass/${id}`)
