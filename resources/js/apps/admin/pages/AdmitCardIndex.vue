        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-5">
            <div class="grid grid-cols-1 gap-4 lg:grid-cols-12">
                ...
                <div class="lg:col-span-2">
                    ...
                    <input v-model="filters.value" type="text" placeholder="Name" class="mt-1 h-10 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100" />
                </div>
                ...
                <table class="min-w-full divide-y divide-slate-200 text-sm">
                    <thead class="bg-slate-50">
                        <tr>
                            ...
                            <th class="px-3 py-2 text-left font-semibold text-slate-700">Admit Card Name</th>
                            <th class="px-3 py-2 text-left font-semibold text-slate-700">Issue</th>
                            <th class="px-3 py-2 text-left font-semibold text-slate-700">Expired</th>
                            <th class="px-3 py-2 text-center font-semibold text-slate-700">Action</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-200 bg-white">
                        <tr v-for="row in rows" :key="'ac-' + row.id">
                            <td class="px-3 py-2 text-slate-800">{{ row.academic_session_name || '--' }}</td>
                            <td class="px-3 py-2 text-slate-800">{{ row.department_name || '--' }}</td>
                            <td class="px-3 py-2 text-slate-800">{{ (row.academic_qualification_name || '') + (row.academic_class_name ? ' (' + row.academic_class_name + ')' : '') }}</td>
                            <td class="px-3 py-2 text-slate-800">{{ row.exam_name || '--' }}</td>
                            <td class="px-3 py-2 text-slate-800">{{ row.name || '--' }}</td>
                            <td class="px-3 py-2 text-slate-800">{{ row.issue_date || '--' }}</td>
                            <td class="px-3 py-2 text-slate-800">{{ row.expired_date || '--' }}</td>
                            <td class="px-3 py-2 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <button type="button" class="rounded-lg border border-slate-200 bg-white px-3 py-1 text-xs font-semibold text-slate-700 hover:bg-slate-50" @click="goView(row.id)">View</button>
                                    <button type="button" class="rounded-lg border border-slate-200 bg-white px-3 py-1 text-xs font-semibold text-slate-700 hover:bg-slate-50" @click="goEdit(row.id)">Edit</button>
                                    <button type="button" class="rounded-lg bg-rose-600 px-3 py-1 text-xs font-semibold text-white hover:bg-rose-700" :disabled="deletingId === row.id" @click="destroyRow(row.id)">
