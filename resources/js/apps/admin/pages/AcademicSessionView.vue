    ...
    <div class="flex flex-col gap-4">
        <div class=" border border-slate-300 bg-white p-5">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div class="min-w-0">
                    <div class="truncate text-xl font-semibold text-slate-900">Academic Session Details</div>
                </div>
                ...
                <div class="flex flex-wrap items-center gap-2">
                    ...
                    <button type="button" class="rounded-sm bg-slate-900 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-800" :disabled="loading || !sessionId" @click="goEdit">Edit</button>
                </div>
            </div>
        </div>
        ...
        <div class=" border border-slate-300 bg-white p-5">
            ...
            <div v-else class="grid grid-cols-1 gap-3 sm:grid-cols-2">
                <div class="rounded-xl border border-slate-300 p-4">
                    <div class="text-xs font-semibold text-slate-500">Session Name</div>
                ...
                <div class="rounded-xl border border-slate-300 p-4">
                    <div class="text-xs font-semibold text-slate-500">Current Financial Year</div>
                    <div class="mt-1 text-sm font-semibold text-slate-900">{{ yesNo(row.current) }}</div>
                </div>
...
export default {
    ...
    methods: {
        ...
        goEdit() {
            this.go(`/admin/academicSession/${this.sessionId}/edit`)
        },
        yesNo(v) {
        ...
        async load() {
            ...
            try {
                ...
                this.row = res?.data?.academic_session || res?.data || {}
            } catch (e) {
