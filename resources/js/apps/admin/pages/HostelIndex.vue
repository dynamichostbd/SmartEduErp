    ...
    <div class="flex flex-col gap-4">
        <div class="rounded-2xl border border-slate-200 bg-white p-5">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                <div>
                    <div class="text-xl font-semibold text-slate-900">Hostel</div>
                </div>
                ...
                <div class="flex w-full flex-col gap-2 sm:w-auto sm:flex-row">
                    ...
                    <input v-model="filters.value" type="text" class="h-10 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm outline-none sm:w-72" placeholder="Search by name..." :disabled="loading" @keyup.enter="load(1)" />
                ...
                <table class="min-w-full border-collapse border border-slate-200">
                    <thead class="bg-slate-50">
                        <tr class="text-left text-xs font-semibold uppercase tracking-wider text-slate-500">
                            ...
                            <th class="border border-slate-200 bg-slate-50 px-4 py-3">Hostel Name</th>
                            <th class="border border-slate-200 bg-slate-50 px-4 py-3">Hostel Fees</th>
                    ...
                    <tbody>
                        ...
                        <tr v-for="(row, idx) in rows" v-else :key="row.id" class="text-sm text-slate-800" :class="idx % 2 === 0 ? 'bg-white' : 'bg-slate-50'">
                            ...
                            <td class="border border-slate-200 px-4 py-3">{{ row.fees ?? '—' }}</td>
...
export default {
    ...
    methods: {
        ...
        goCreate() {
            this.go('/admin/hostel/create')
        },
        goView(id) {
            this.go(`/admin/hostel/${id}`)
        },
        goEdit(id) {
        ...
        async load(page = 1) {
            ...
            this.error = ''
            try {
                const res = await window.axios.get('/admin/hostel', {
                    params: {
                        page,
                ...
                this.error = e?.response?.data?.message || 'Failed to load hostels.'
            } finally {
                this.loading = false
            }
        },
        async destroy(id) {
            ...
            this.deletingId = id
            try {
                const res = await window.axios.delete(`/admin/hostel/${id}`)
