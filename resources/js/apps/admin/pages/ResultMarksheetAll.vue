<template>
    <div class="flex flex-col gap-4">
        <div class="rounded-2xl border border-slate-200 bg-white p-5 print:hidden">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div class="flex flex-wrap items-center gap-2">
                    ...
                    <button type="button" class="rounded-lg bg-emerald-600 px-4 py-2 text-sm font-semibold text-white hover:bg-emerald-700" :disabled="printing" @click="print">{{ printing ? '...' : 'PRINT' }}</button>
        ...
        <div v-if="loading" class="rounded-2xl border border-slate-200 bg-white p-6 text-sm text-slate-600">Loading...</div>
        ...
        <div v-else class="flex flex-col gap-4">
            ...
            <div v-for="(it, idx) in (items || [])" :key="'mk-all-' + (it?.id || idx)" class="rounded-2xl border border-slate-200 bg-white p-4 sm:p-6">
                ...
                <div class="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-2 text-sm text-slate-700">
                    <div><span class="font-semibold">Software ID</span> {{ it?.student?.student_id || '' }}</div>
                    <div><span class="font-semibold">Session</span> {{ it?.result?.academic_session?.name || '' }}</div>
                    <div><span class="font-semibold">Name</span> {{ it?.student?.name || '' }}</div>
                    <div><span class="font-semibold">Academic Level</span> {{ it?.result?.qualification?.name || '' }}</div>
                    <div><span class="font-semibold">College Roll</span> {{ it?.student?.college_roll || '' }}</div>
                    <div><span class="font-semibold">Dept / Group</span> {{ it?.result?.department?.name || '' }}</div>
                    <div><span class="font-semibold">Exam</span> {{ it?.result?.exam?.name || '' }}</div>
                    <div><span class="font-semibold">Class</span> {{ it?.result?.academic_class?.name || '' }}</div>
                </div>
