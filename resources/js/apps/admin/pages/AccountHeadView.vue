    ...
    <div class="flex flex-col gap-4">
        <div class="rounded-2xl border border-slate-200 bg-white p-5">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div class="min-w-0">
                    <div class="truncate text-xl font-semibold text-slate-900">Account Head Details</div>
                </div>
                ...
                <div class="flex flex-wrap items-center gap-2">
                    ...
                    <button type="button" class="rounded-lg bg-slate-900 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-800" :disabled="loading || !accountHeadId" @click="goEdit">Edit</button>
                </div>
            </div>
        </div>
        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-5">
            ...
            <div v-else class="grid grid-cols-1 gap-3 sm:grid-cols-2">
                ...
                <div class="rounded-xl border border-slate-200 p-4">
                    ...
                    <div class="mt-1 text-sm font-semibold text-slate-900">{{ row.type || '—' }}</div>
                </div>
                <div class="rounded-xl border border-slate-200 p-4">
...
export default {
    ...
    methods: {
        ...
        goEdit() {
            this.go(`/admin/accountHead/${this.accountHeadId}/edit`)
        },
        async load() {
            if (!this.accountHeadId) return
            this.loading = true
            this.error = ''
            try {
                const res = await window.axios.get(`/admin/accountHead/${this.accountHeadId}`)
                this.row = res?.data?.account_head || res?.data || {}
            } catch (e) {
