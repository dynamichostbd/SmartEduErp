    ...
    <div class="flex flex-col gap-4">
        <div class=" border border-slate-300 bg-white p-5">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                <div>
                    <div class="text-xl font-semibold text-slate-900">Fee Setup</div>
                </div>

                <div class="flex w-full flex-col gap-2 sm:w-auto sm:flex-row">
                    <select v-model="filters.academic_qualification_id" class="h-9 rounded-sm border border-slate-300 bg-white px-3 text-sm outline-none" :disabled="loading">
                        <option value="">All Academic Level</option>
                        <option v-for="q in qualifications" :key="'q-' + q.id" :value="String(q.id)">{{ q.name }}</option>
                    </select>

                    <select v-model="filters.department_id" class="h-9 rounded-sm border border-slate-300 bg-white px-3 text-sm outline-none" :disabled="loading || !filters.academic_qualification_id">
                        <option value="">All Department</option>
                        <option v-for="d in departmentsFiltered" :key="'d-' + d.id" :value="String(d.id)">{{ d.name }}</option>
                    </select>

                    <select v-model="filters.academic_class_id" class="h-9 rounded-sm border border-slate-300 bg-white px-3 text-sm outline-none" :disabled="loading || !filters.academic_qualification_id">
                ...
                <table class="min-w-full border-collapse border border-slate-300">
                    <thead class="bg-emerald-100">
                        <tr class="text-left text-xs font-semibold uppercase tracking-wider text-slate-700">
                            ...
                            <th class="border border-slate-300 bg-emerald-100 px-1 py-2">Department</th>
                            <th class="border border-slate-300 bg-emerald-100 px-1 py-2">Qualification</th>
                            <th class="border border-slate-300 bg-emerald-100 px-1 py-2">Class Name</th>
                            <th class="border border-slate-300 bg-emerald-100 px-1 py-2">Created at</th>
                    ...
                    <tbody>
                        ...
                        <tr v-for="(row, idx) in rows" v-else :key="row.id" class="text-sm text-slate-800" :class="idx % 2 === 0 ? 'bg-white' : 'bg-slate-50'">
                            ...
                            <td class="border border-slate-300 px-1 py-1">{{ row.qualification_name || '—' }}</td>
                            <td class="border border-slate-300 px-1 py-1">{{ row.class_name || '—' }}</td>
                            <td class="border border-slate-300 px-1 py-1">{{ row.created_at || '—' }}</td>
...
export default {
    ...
    methods: {
        ...
        goCreate() {
            this.go('/admin/feeSetup/create')
        },
        goView(id) {
            this.go(`/admin/feeSetup/${id}`)
        },
        goEdit(id) {
            this.go(`/admin/feeSetup/${id}/edit`)
        },
        ...
        async load(page = 1) {
            ...
            this.error = ''
            try {
                const res = await window.axios.get('/admin/feeSetup', {
                    params: {
                        page,
                ...
                this.error = e?.response?.data?.message || 'Failed to load fee setups.'
            } finally {
                this.loading = false
            }
        },
        async destroy(id) {
            ...
            this.deletingId = id
            try {
                const res = await window.axios.delete(`/admin/feeSetup/${id}`)
