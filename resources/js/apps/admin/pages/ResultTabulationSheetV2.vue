<template>
    <div class="flex flex-col gap-4">
        <div class="rounded-2xl border border-slate-200 bg-white p-5 print:hidden">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div class="flex flex-wrap items-center gap-2">
                    ...
                    <button type="button" class="rounded-lg bg-slate-900 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-800" :disabled="printing || !hasPages" @click="print">{{ printing ? '...' : 'PRINT' }}</button>
                </div>
            </div>
        </div>
        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-5 print:hidden">
            <div class="grid grid-cols-1 gap-4 lg:grid-cols-12">
                ...
                <div class="lg:col-span-3">
                    ...
                    <input v-model="filters.search_keyword" type="text" class="mt-1 h-10 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100" placeholder="Type your text" @keydown.enter.prevent="search" />
                </div>
        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-5" id="printArea">
            <div v-if="hasPages" class="space-y-6">
                <div v-for="(subjectChunk, chunkIndex) in result.subject_chunks" :key="'chunk-' + chunkIndex" class="tabulation-page" :class="{ 'page-break': chunkIndex < result.subject_chunks.length - 1 }">
                    <table class="mb-3 w-full overflow-hidden rounded-xl border border-slate-200 text-sm">
                        <tbody>
                        ...
                        <table class="min-w-full border-collapse text-xs">
                            <thead class="bg-slate-50">
                                <tr>
                                    ...
                                    <th rowspan="2" class="border border-slate-300 px-2 py-2 text-left font-semibold">Software ID</th>
                                    ...
                                    <th rowspan="2" class="border border-slate-300 px-2 py-2 text-left font-semibold">Mobile</th>

                                    <template v-if="subjectChunk.length">
                                        <th v-for="(sub, sbKey) in subjectChunk" :key="`sub_${chunkIndex}_${sbKey}`" colspan="7" class="border border-slate-300 px-2 py-2 text-center font-semibold">
                                ...
                                <tr v-if="subjectChunk.length" class="text-center">
                                    <template v-for="(sub, sbKey) in subjectChunk" :key="`subh_${chunkIndex}_${sbKey}`">
                                        ...
                                        <th class="border border-slate-300 px-2 py-2 font-semibold">CONVERT</th>
                                        <th class="border border-slate-300 px-2 py-2 font-semibold">TOTAL</th>
                            ...
                            <tbody v-if="(result.result_sheet || []).length" class="bg-white">
                                <tr v-for="(std, idx) in result.result_sheet" :key="'std-' + chunkIndex + '-' + idx">
                                    ...
                                    <td class="border border-slate-300 px-2 py-1">{{ std.software_id }}</td>
                                    <td class="border border-slate-300 px-2 py-1">{{ std.college_roll }}</td>
                                    <td class="border border-slate-300 px-2 py-1">{{ std.name }}</td>
                                    <td class="border border-slate-300 px-2 py-1">{{ std.mobile }}</td>

                                    <template v-if="std.subject_chunks && std.subject_chunks[chunkIndex] && std.subject_chunks[chunkIndex].length">
                                        <template v-for="(mark, rmKey) in std.subject_chunks[chunkIndex]" :key="'mk-' + chunkIndex + '-' + rmKey">
                                            <td class="border border-slate-300 px-2 py-1 text-center">{{ mark.ct_mark }}</td>
                                            ...
                                            <td class="border border-slate-300 px-2 py-1 text-center">{{ mark.obtained_mark }}</td>
                                    ...
                                    <template v-else>
                                        <template v-for="(sub, subIdx) in subjectChunk" :key="'empty-' + subIdx">
                            ...
                            <tbody v-else>
                                <tr>
                                    <td :colspan="5 + subjectChunk.length * 7 + 3" class="border border-slate-300 px-3 py-10 text-center text-slate-500">No data Found</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>

                    <div class="mt-2 text-right text-[11px] text-slate-700">Page {{ chunkIndex + 1 }} of {{ result.subject_chunks.length }}</div>
                </div>
            </div>

            <div v-else class="rounded-xl border border-slate-200 bg-slate-50 p-8 text-center text-sm text-slate-600">No result found.</div>
        </div>
    </div>
</template>
...
export default {
    ...
    computed: {
        hasPages() {
            return Array.isArray(this.result?.subject_chunks) && this.result.subject_chunks.length > 0
        },
        sessions() {
    ...
    methods: {
        ...
        async search() {
            ...
            this.loading = true
            try {
                const res = await window.axios.get('/admin/tabulation-sheet-v2-data', { params: this.filters })
        ...
        print() {
            ...
            styleEl.setAttribute('data-print-page-style', 'tabulation-sheet-v2')
