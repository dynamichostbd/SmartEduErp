    ...
    <div class="flex flex-col gap-4">
        <div class=" border border-slate-300 bg-white p-5">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                <div>
                    <div class="text-xl font-semibold text-slate-900">Subject</div>
                </div>
        ...
        <div class="overflow-hidden  border border-slate-300 bg-white">
            <div class="overflow-x-auto">
                <table class="min-w-[600px] w-full border-collapse border border-slate-300">
                    <thead class="bg-emerald-100">
                        <tr class="text-left text-xs font-semibold uppercase tracking-wider text-slate-700">
                            ...
                            <th class="border border-slate-300 bg-emerald-100 px-1 py-2 text-slate-700">Name (en)</th>
                            <th class="border border-slate-300 bg-emerald-100 px-1 py-2 text-slate-700">Name (bn)</th>
                            <th class="border border-slate-300 bg-emerald-100 px-1 py-2 text-center text-slate-700">Sorting</th>
                    ...
                    <tbody>
                        <tr v-if="loading" class="text-sm text-slate-600">
                            <td class="border border-slate-300 px-1 py-1" colspan="5">Loading...</td>
                        </tr>
                        <tr v-else-if="rows.length === 0" class="text-sm text-slate-600">
                            <td class="border border-slate-300 px-1 py-1" colspan="5">No data found.</td>
                        </tr>
                        <tr v-for="(row, idx) in rows" :key="row.id" :class="idx % 2 === 0 ? 'bg-white' : 'bg-slate-50'">
                            ...
                            <td class="border border-slate-300 px-1 py-1 text-slate-800">{{ row.name_en || '' }}</td>
                            <td class="border border-slate-300 px-1 py-1 text-slate-800">{{ row.name_bn || '' }}</td>
                            <td class="border border-slate-300 px-1 py-1 text-center">{{ row.sorting || '' }}</td>
...
export default {
    ...
    computed: {
        ...
        canCreate() {
            return !this.perms.length || this.perms.includes('subject.create')
        },
        canEdit() {
            return !this.perms.length || this.perms.includes('subject.edit')
        },
        canDelete() {
            return !this.perms.length || this.perms.includes('subject.destroy')
        },
    },
    created() {
        this.load(1)
    ...
    methods: {
        ...
        goCreate() {
            this.go('/admin/subject/create')
        },
        goView(id) {
            this.go(`/admin/subject/${id}`)
        },
        goEdit(id) {
        ...
        async load(page = 1) {
            ...
            this.error = ''
            try {
                const res = await window.axios.get('/admin/subject', {
                    params: {
                        page,
        ...
        async destroy(id) {
            ...
            this.deletingId = id
            try {
                const res = await window.axios.delete(`/admin/subject/${id}`)
