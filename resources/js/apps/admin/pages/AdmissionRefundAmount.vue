    ...
    <div class="flex flex-col gap-4">
        <div class="rounded-2xl border border-slate-200 bg-white p-5">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div class="min-w-0">
                    <div class="truncate text-xl font-semibold text-slate-900">Admission Refund Amount</div>
            ...
            <div class="overflow-x-auto rounded-xl border border-slate-200">
                <table class="min-w-full divide-y divide-slate-200 text-sm">
                    ...
                    <tbody class="divide-y divide-slate-200 bg-white">
                        <tr v-for="row in rows" :key="'r-' + row.id">
                            <td class="px-3 py-2">
                                ...
                                <div class="text-xs text-slate-600">{{ row.admission_roll }}</div>
                            ...
                            <td class="px-3 py-2 text-right font-semibold">{{ money(row.refund_amount) }}</td>
                    ...
                    <tfoot class="bg-slate-50">
                        <tr>
                            <td colspan="5" class="px-3 py-2 text-right font-semibold text-slate-700">Refund Amount</td>
                            <td class="px-3 py-2 text-right font-semibold text-slate-900">{{ money(totalRefundAmount) }}</td>
                            <td></td>
                        </tr>
                        <tr>
                            <td colspan="6" class="px-3 py-2 text-right font-semibold text-slate-700">Total Amount</td>
...
export default {
    ...
    computed: {
        ...
        totalRefundAmount() {
            return this.rows.reduce((sum, r) => sum + (Number(r?.refund_amount || 0) || 0), 0)
        },
    },
    created() {
        this.load(1)
    ...
    methods: {
        ...
        async load(page) {
            ...
            try {
                ...
                const res = await window.axios.get('/admin/admission-refund-amount', { params })
