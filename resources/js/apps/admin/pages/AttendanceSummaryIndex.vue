        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-5">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div class="min-w-0">
                    ...
                    <div class="mt-1 text-sm text-slate-600">Summary list</div>
                </div>
            ...
            <div class="overflow-x-auto rounded-xl border border-slate-200">
                <table class="min-w-full divide-y divide-slate-200 text-sm">
                    <thead class="bg-slate-50">
                        <tr>
                            <th class="px-3 py-2 text-left font-semibold text-slate-700">From Date</th>
                            <th class="px-3 py-2 text-left font-semibold text-slate-700">To Date</th>
                    ...
                    <tbody class="divide-y divide-slate-200 bg-white">
                        <tr v-for="row in rows" :key="'s-' + row.id">
                            <td class="px-3 py-2 text-slate-800">{{ row.from_date || '--' }}</td>
                            <td class="px-3 py-2 text-slate-800">{{ row.to_date || '--' }}</td>
                            ...
                            <td class="px-3 py-2 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <button type="button" class="rounded-lg border border-slate-200 bg-white px-3 py-1 text-xs font-semibold text-slate-700 hover:bg-slate-50" @click="goView(row.id)">
                                        View
                                    </button>
                                    <button type="button" class="rounded-lg border border-slate-200 bg-white px-3 py-1 text-xs font-semibold text-slate-700 hover:bg-slate-50" @click="goEdit(row.id)">
                                        Edit
                                    </button>
...
export default {
    ...
    methods: {
        ...
        goCreate() {
            window.location.href = '/admin/attendanceSummary/create'
        },
        goView(id) {
            window.location.href = `/admin/attendanceSummary/${id}`
        },
        goEdit(id) {
            window.location.href = `/admin/attendanceSummary/${id}/edit`
        },
        goPage(p) {
        ...
        async search(resetPage) {
            ...
            this.error = ''
            try {
                const res = await window.axios.get('/admin/attendanceSummary/list', { params: this.filters })
        ...
        async destroyRow(id) {
            ...
            this.error = ''
            try {
                await window.axios.delete(`/admin/attendanceSummary/${id}`)
