        ...
        <div class=" border border-slate-300 bg-white p-5">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div class="min-w-0">
                    ...
                    <div class="mt-1 text-sm text-slate-600">Class test result list</div>
                </div>
                ...
                <table class="min-w-full border-collapse border border-slate-300 text-sm">
                    <thead class="bg-emerald-100">
                        <tr class="text-left text-xs font-semibold uppercase tracking-wider text-slate-700">
                            ...
                            <th class="border border-slate-300 bg-emerald-100 px-1 py-2 text-slate-700">Exam Date</th>
                    ...
                    <tbody class="bg-white">
                        <tr v-for="(row, idx) in rows" :key="'ctr-' + row.id" :class="idx % 2 === 0 ? 'bg-white' : 'bg-slate-50'">
                            ...
                            <td class="border border-slate-300 px-3 py-2 text-slate-800">{{ row.exam_date || '--' }}</td>
...
export default {
    ...
    methods: {
        ...
        goCreate() {
            window.location.href = '/admin/classTestResult/create'
        },
        goView(id) {
            window.location.href = `/admin/classTestResult/${id}`
        },
        goEdit(id) {
        ...
        async search(resetPage) {
            ...
            this.error = ''
            try {
                const res = await window.axios.get('/admin/classTestResult/list', { params: this.filters })
                this.rows = Array.isArray(res?.data?.data) ? res.data.data : Array.isArray(res?.data?.data?.data) ? res.data.data.data : []
                this.meta = res?.data?.meta || res?.data || this.meta
                ...
                this.error = e?.response?.data?.message || 'Failed to load class test results.'
                this.rows = []
            } finally {
                this.loading = false
            }
        ...
        async sync(id) {
            ...
            this.error = ''
            try {
                const res = await window.axios.get(`/admin/classTestResult-sync/${id}`)
                const msg = res?.data?.message || 'Result Sync Successfully!'
                this.$toast?.success?.(msg)
                await this.search(false)
            } catch (e) {
                this.error = e?.response?.data?.message || 'Failed to sync result.'
        ...
        async togglePublished(row) {
            ...
            this.error = ''
            try {
                const res = await window.axios.post('/admin/classTestResult-published', { result_id: row.id, published_date: pubDate })
                const msg = res?.data?.message || 'Published Successfully!'
                this.$toast?.success?.(msg)
                await this.search(false)
            } catch (e) {
                this.error = e?.response?.data?.message || 'Failed to publish/unpublish.'
                this.$toast?.error?.(this.error)
            } finally {
        ...
        async destroyRow(row) {
            ...
            this.error = ''
            try {
                const res = await window.axios.delete(`/admin/classTestResult/${row.id}`)
