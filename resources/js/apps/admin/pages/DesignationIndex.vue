    ...
    <div class="flex flex-col gap-4">
        <div class=" border border-slate-300 bg-white p-5">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                <div>
                    <div class="text-xl font-semibold text-slate-900">Designation</div>
                </div>
                ...
                <table class="min-w-full border-collapse border border-slate-300">
                    <thead class="bg-emerald-100">
                        <tr class="text-left text-xs font-semibold uppercase tracking-wider text-slate-700">
                            ...
                            <th class="border border-slate-300 bg-emerald-100 px-1 py-2 text-slate-700">Short Name</th>
...
export default {
    ...
    computed: {
        ...
        canCreate() {
            return !this.perms.length || this.perms.includes('designation.create')
        },
        canEdit() {
            return !this.perms.length || this.perms.includes('designation.edit')
        },
        canDelete() {
            return !this.perms.length || this.perms.includes('designation.destroy')
        },
    },
    created() {
        this.load(1)
    ...
    methods: {
        ...
        goCreate() {
            this.go('/admin/designation/create')
        },
        goView(id) {
            this.go(`/admin/designation/${id}`)
        },
        goEdit(id) {
        ...
        async load(page = 1) {
            ...
            this.error = ''
            try {
                const res = await window.axios.get('/admin/designation', {
                    params: {
                        page,
                ...
                this.error = e?.response?.data?.message || 'Failed to load designations.'
            } finally {
                this.loading = false
            }
        },
        async destroy(id) {
            ...
            this.deletingId = id
            try {
                const res = await window.axios.delete(`/admin/designation/${id}`)
