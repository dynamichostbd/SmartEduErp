        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-5">
            ...
            <div class="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-12">
                ...
                <div class="lg:col-span-4">
                    <img v-if="data.image && String(data.file_type || '') === 'Image'" :src="resolveFile(data.image)" class="h-56 w-full rounded border border-slate-200 bg-white object-contain" />
                    <a v-else-if="data.image" :href="resolveFile(data.image)" class="text-emerald-700 hover:underline" target="_blank">View File</a>
                    <div v-else class="flex h-56 items-center justify-center rounded border border-dashed border-slate-200 bg-slate-50 text-xs text-slate-500">No file</div>
                </div>
            </div>
        </div>
    </div>
