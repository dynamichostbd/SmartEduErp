            ...
            <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                ...
                <div class="flex w-full flex-col gap-2 sm:w-auto sm:flex-row">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <button v-if="canCreate" class="h-10 rounded-lg bg-slate-900 px-4 text-sm font-semibold text-white hover:bg-slate-800" :disabled="loading" @click="goCreate">
                            Add
                        </button>

                        <button class="h-10 rounded-lg border border-slate-200 bg-white px-4 text-sm font-semibold text-slate-700 hover:bg-slate-50" :disabled="loading" @click="reset">
                            Reset
                        </button>
                    </div>
                </div>
                ...
                <table class="min-w-full border-collapse border border-slate-200">
                    <thead class="bg-slate-50">
                        <tr class="text-left text-xs font-semibold uppercase tracking-wider text-slate-500">
                            ...
                            <th class="border border-slate-200 bg-slate-50 px-4 py-3 text-center">File</th>
                    ...
                    <tbody>
                        ...
                        <tr v-for="(row, idx) in rows" v-else :key="'r-' + row.id" class="text-sm text-slate-800" :class="idx % 2 === 0 ? 'bg-white' : 'bg-slate-50'">
                            ...
                            <td class="border border-slate-200 px-4 py-3 text-center">{{ row.file_type || '—' }}</td>
                            <td class="border border-slate-200 px-4 py-3 text-center">
                                <a v-if="row.file" class="text-emerald-700 hover:underline" :href="resolveFile(row.file)" target="_blank">View</a>
                                <span v-else>—</span>
                            </td>
                            <td class="border border-slate-200 px-4 py-3 text-center">
