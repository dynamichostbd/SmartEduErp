    ...
    <form class="flex flex-col gap-4" @submit.prevent="submit">
        <div class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div class="min-w-0">
                    ...
                    <div class="mt-1 text-sm text-slate-600">Configure popup title, description and image</div>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 gap-4 lg:grid-cols-12">
            <div class="flex flex-col gap-4 lg:col-span-8">
                <div class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
                    <div class="text-sm font-semibold text-slate-900">Popup Details</div>
                    ...
                    <div class="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                        ...
                        <div class="sm:col-span-2 lg:col-span-3">
                            <div class="mt-2 rounded-xl border border-slate-200 bg-slate-50 p-3">
                                ...
                                <div class="mt-2">
                                    <img v-if="previewUrl" :src="previewUrl" class="h-40 w-full rounded-lg border border-slate-200 bg-white object-contain" />
                                    <img v-else-if="form.existing_image" :src="resolveImage(form.existing_image)" class="h-40 w-full rounded-lg border border-slate-200 bg-white object-contain" />
                                    <div v-else class="flex h-40 items-center justify-center rounded-lg border border-dashed border-slate-300 bg-slate-50 text-xs text-slate-500">No image</div>
                                </div>
                            </div>
                        </div>
                    </div>
