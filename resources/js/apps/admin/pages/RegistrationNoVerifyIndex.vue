    ...
    <div class="flex flex-col gap-4">
        <div class=" border border-slate-300 bg-white p-5">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div class="min-w-0">
                    ...
                    <div class="mt-1 text-sm text-slate-600">Manage registration number lists</div>
                </div>
        ...
        <div class="overflow-hidden border border-slate-300 bg-white">
            <div class="overflow-x-auto">
                <table class="min-w-[800px] w-full border-collapse border border-slate-300 text-sm">
                    <thead class="bg-emerald-100">
                        <tr class="text-left text-xs font-semibold uppercase tracking-wider text-slate-700">
                            ...
                            <th class="border border-slate-300 bg-slate-50 px-1 py-1 text-xs whitespace-nowrap">Academic Level</th>
                            <th class="border border-slate-300 bg-slate-50 px-1 py-1 text-xs whitespace-nowrap">Dept. / Group</th>
                            <th class="border border-slate-300 bg-slate-50 px-1 py-1 text-xs whitespace-nowrap">Academic Class</th>
                    ...
                    <tbody>
                        <tr v-for="(row, idx) in rows" :key="'row-' + row.id" class="text-sm text-slate-800" :class="idx % 2 === 0 ? 'bg-white' : 'bg-slate-50'">
                            ...
                            <td class="border border-slate-300 px-1 py-1">{{ row.academic_qualification_name || '--' }}</td>
                            <td class="border border-slate-300 px-1 py-1">{{ row.department_name || '--' }}</td>
                            <td class="border border-slate-300 px-1 py-1">{{ row.academic_class_name || '--' }}</td>
                        ...
                        <tr v-if="!rows.length" class="text-sm text-slate-600">
                            <td colspan="5" class="border border-slate-300 px-1 py-10 text-center">{{ loading ? 'Loading...' : 'No data found' }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
            ...
            <div v-if="meta.total" class="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div class="flex flex-wrap items-center gap-1">
                    <button type="button" class="h-9 rounded-sm border border-slate-300 bg-white px-3 text-sm font-semibold text-slate-700 hover:bg-slate-50" :disabled="filters.page <= 1 || loading" @click="goPage(filters.page - 1)">
                        Prev
                    </button>
                    ...
                    <button type="button" class="h-9 rounded-sm border border-slate-300 bg-white px-3 text-sm font-semibold text-slate-700 hover:bg-slate-50" :disabled="filters.page >= meta.last_page || loading" @click="goPage(filters.page + 1)">
                        Next
                    </button>
                </div>
            </div>
...
<script>
export default {
    name: 'RegistrationNoVerifyIndex',
    props: {
        systems: {
            type: Object,
            default: () => ({}),
    ...
    methods: {
        ...
        goCreate() {
            window.location.href = '/admin/registrationNoVerify/create'
        },
        goView(id) {
            window.location.href = `/admin/registrationNoVerify/${id}`
        },
        goPage(p) {
        ...
        async search(resetPage) {
            ...
            this.error = ''
            try {
                const res = await window.axios.get('/admin/registrationNoVerify/list', { params: this.filters })
        ...
        async destroyRow(id) {
            ...
            this.error = ''
            try {
                await window.axios.delete(`/admin/registrationNoVerify/${id}`)
                this.search(true)
            } catch (e) {
