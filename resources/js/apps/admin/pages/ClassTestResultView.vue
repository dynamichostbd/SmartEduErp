        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-5">
            <div class="grid grid-cols-1 gap-4 lg:grid-cols-12">
                <div class="lg:col-span-3">
                    ...
                    <select v-model="filters.type" class="mt-1 h-10 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100" @change="load(true)">
                ...
                <div class="lg:col-span-3">
                    ...
                    <select v-model="filters.field_name" class="mt-1 h-10 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100" @change="load(true)">
                ...
                <div class="lg:col-span-4">
                    ...
                    <input v-model="filters.value" type="text" class="mt-1 h-10 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100" @keydown.enter.prevent="load(true)" />
                </div>

                <div class="lg:col-span-2 flex items-end">
                    <button type="button" class="h-10 w-full rounded-lg bg-slate-900 px-4 text-sm font-semibold text-white hover:bg-slate-800" :disabled="loading" @click="load(true)">{{ loading ? '...' : 'Go' }}</button>
                </div>
            </div>
        </div>
        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-5">
            <div class="mb-4 rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-700">
                <div class="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-5">
                    ...
                    <div><span class="font-semibold">Dept:</span> {{ result?.department?.name || result?.department_name || '--' }}</div>
                    <div><span class="font-semibold">Class:</span> {{ result?.academic_class?.name || result?.academic_class_name || '--' }}</div>
                    <div><span class="font-semibold">Exam:</span> {{ result?.exam?.name || result?.exam_name || '--' }}</div>
                </div>
            </div>
                ...
                <table id="pdf-table" class="min-w-full divide-y divide-slate-200 text-sm">
                    ...
                    <tbody class="divide-y divide-slate-200 bg-white">
                        ...
                        <tr v-if="!(result?.details || []).length">
                            <td :colspan="filters.type === 'unmerit' ? 8 : 6" class="px-3 py-10 text-center text-slate-500">No data found</td>
                        </tr>
                    </tbody>
                </table>
            </div>
...
export default {
    ...
    methods: {
        ...
        goMarksheet(detailId) {
            window.location.href = `/admin/classTestResult-marksheet/${detailId}`
        },
        async load(reset) {
            if (reset) {
            ...
            this.error = ''
            try {
                const res = await window.axios.get(`/admin/classTestResult/${this.resultId}/details`, { params: this.filters })
        ...
        async exportPdf() {
            ...
            try {
                ...
                doc.save('class_test_result.pdf')
            } finally {
