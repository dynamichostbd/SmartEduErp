...
class ClassRoutineController extends Controller
{
    ...
    {
        return Schema::hasTable('class_routines') ? 'class_routines' : null;
    }

    private function uploadDir(): string
    {
        $collegeName = env('COLLEGE_NAME', 'default_college');
        return "upload/{$collegeName}/class_routine";
    }
    ...
    public function store(Request $request)
    {
        $table = $this->table();
        if (!$table) {
            return response()->json(['message' => 'Class Routine module not ready'], 422);
        }
