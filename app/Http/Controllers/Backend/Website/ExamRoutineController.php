...
class ExamRoutineController extends Controller
{
    ...
    {
        return Schema::hasTable('exam_routines') ? 'exam_routines' : null;
    }

    private function uploadDir(): string
    {
        $collegeName = env('COLLEGE_NAME', 'default_college');
        return "upload/{$collegeName}/exam_routine";
    }
    ...
    public function store(Request $request)
    {
        $table = $this->table();
        if (!$table) {
            return response()->json(['message' => 'Exam Routine module not ready'], 422);
        }
