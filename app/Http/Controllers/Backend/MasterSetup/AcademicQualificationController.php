...
class AcademicQualificationController extends Controller
{
    ...
    {
        return Schema::hasTable('academic_qualifications') ? 'academic_qualifications' : null;
    }
    ...
    public function store(Request $request)
    {
        $table = $this->table();
        if (!$table) {
            return response()->json(['message' => 'Academic Level module not ready'], 422);
        }
        ...
        foreach (['application_fees', 'admission_roll_verify', 'subject_choose', 'migration', 'registration', 'online_admission', 'application_fee'] as $flag) {
            if (in_array($flag, $columns, true)) {
                $payload[$flag] = (int) ($request->input($flag) ? 1 : 0);
            }
        }

        if (in_array('admission_files', $columns, true)) {
            $selected = $request->input('selected_admission_files');
            $payload['admission_files'] = !empty($selected) ? json_encode($selected) : null;
        }
    ...
    public function show(Request $request, $id)
    {
        ...
        return response()->json(['academic_qualification' => $row]);
    }
