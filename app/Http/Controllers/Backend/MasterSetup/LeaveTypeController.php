...
class LeaveTypeController extends Controller
{
    ...
    {
        return Schema::hasTable('leave_types') ? 'leave_types' : null;
    }
    ...
    public function index(Request $request)
    {
        ...
        if ($value !== '' && in_array($field, ['name', 'short_name'], true) && in_array($field, $cols, true)) {
    ...
    public function store(Request $request)
    {
        $table = $this->table();
        if (!$table) {
            return response()->json(['message' => 'Leave Type module not ready'], 422);
        }
        ...
        foreach (['back_date_select', 'documents_upload', 'reason'] as $flag) {
    ...
    public function show(Request $request, $id)
    {
        ...
        return response()->json(['leave_type' => $row], 200);
    }
